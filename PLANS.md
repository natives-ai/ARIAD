...
### 2026-05-01 / loop-237
- Active milestone: Deployment packaging support
- Agents engaged: Orchestrator, Backend, Frontend, Version Control
- Touched zones: `.dockerignore`, `backend/Dockerfile`, `frontend/Dockerfile`, `docker-compose.ec2.yml`, `deploy/ec2.env.example`, `docs/EC2_DOCKER_DEPLOY.md`, `backend/src/config/env.ts`, `backend/src/server.ts`, `backend/.env.example`, `PLANS.md`
- What changed: Added EC2-oriented Docker packaging with separate frontend, backend, and MySQL services. The frontend container serves the built SPA on a public port and proxies `/api` to the internal backend service. The backend image builds shared/recommendation/backend workspaces, runs as the non-root `node` user, and now supports `BACKEND_HOST=0.0.0.0` for container networking while preserving the local `127.0.0.1` default. Added an EC2 env example and deployment runbook covering secrets, Google client ID, HTTP/HTTPS cookie mode, health checks, updates, and volume persistence.
- Tests run: `docker compose --env-file deploy/ec2.env.example -f docker-compose.ec2.yml config` (pass, rerun outside sandbox because Docker config access was blocked in sandbox); `docker compose --env-file deploy/ec2.env.example -f docker-compose.ec2.yml build backend frontend` (pass after Docker Desktop was started); `docker compose -p ariad-smoke --env-file deploy/ec2.env.example -f docker-compose.ec2.yml up -d --wait` with `ARIAD_HTTP_PORT=8080` (pass, mysql/backend/frontend healthy); `Invoke-WebRequest http://127.0.0.1:8080/service.html` (200); `Invoke-WebRequest http://127.0.0.1:8080/api/health` (backend `status: ok`); `corepack yarn workspace @scenaairo/backend typecheck` (pass); `corepack yarn workspace @scenaairo/backend build` (pass); `corepack yarn workspace @scenaairo/frontend build` (pass); `corepack yarn workspace @scenaairo/backend test` (pass, 20 tests); `corepack yarn workspace @scenaairo/recommendation build` (pass); `corepack yarn workspace @scenaairo/shared build` (pass); `git diff --check` (pass, line-ending warnings only)
