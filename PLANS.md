...
- Default to 1?? active milestones at a time when dependencies do not conflict; activating more than 2 requires explicit Orchestrator justification.
...
**Required dependencies**
- Milestones 3?? complete enough for integrated validation
...
**Required dependencies**
- Milestones 1?? complete or intentionally deferred without violating baseline integrity
...
- What changed: Replaced the previously added checklist-style review wrapper with a real second frontend app entry, `service.html`, that mounts the existing ARIAD app directly. Added the minimal Vite multi-page input wiring so the service entry is emitted during build alongside `index.html`.
- Tests run: `npm.cmd run build`
...
- What changed: Added a root-level launcher HTML, `OPEN-ARIAD.html`, so the live service link is immediately visible from the `ARIAD` folder without digging into `frontend/`. Kept the real app entry at `/service.html` unchanged.
- Tests run: `npm.cmd run build`
...
- Agents engaged: Orchestrator
- Touched zones: root workspace, `PLANS.md`
- What changed: Added `OPEN-ARIAD.cmd` as a one-click Windows launcher that starts the backend and frontend dev servers in separate terminals, waits briefly for boot, and opens the live `/service.html` entry automatically. Updated the root helper HTML so it now points users to the `.cmd` launcher first.
- Tests run: `npm.cmd run build`
...
- Approval needed: `none`
- Next step: Optional follow-up only if requested: add PID/port-based reuse logic so repeated launcher runs attach to already-running ARIAD windows instead of opening new ones.

### 2026-04-15 / loop-019
- Active milestone: Post-baseline support task
- Agents engaged: Orchestrator
- Touched zones: root workspace, `PLANS.md`
- What changed: Added a root-level `ARIAD.html` entry that lives directly in the `ARIAD` folder, checks whether the local service at `/service.html` is reachable, automatically forwards into the real ARIAD app when it is, and otherwise stays on a minimal recovery screen that tells the user to run `OPEN-ARIAD.cmd` and retry.
...
- Agents engaged: Orchestrator
- Touched zones: `PLANS.md`
- What changed: Diagnosed the failed local-service launch report without widening into implementation. Confirmed from the current codebase that ARIAD does not require Supabase or another external backend to boot locally, because the frontend is a served React app and the backend is a local Fastify process with file-backed local persistence defaults. Reproduced the real runtime successfully by starting `node backend/dist/server.js` and `node frontend/scripts/serve-dist.mjs`, then confirming both `http://127.0.0.1:3001/api/health` and `http://127.0.0.1:5173/service.html` returned `200`. Re-ran the existing `OPEN-ARIAD.cmd` self-test and confirmed the launcher also reports both backend and frontend ready in this environment.
- Tests run: `node --version`; direct runtime verification by starting `node backend/dist/server.js` and `node frontend/scripts/serve-dist.mjs`, then checking `http://127.0.0.1:3001/api/health` and `http://127.0.0.1:5173/service.html`; launcher self-test: `cmd.exe /c "set ARIAD_NO_BROWSER=1&& call OPEN-ARIAD.cmd"`
...
- Tests run: `npm.cmd run build`; failing proof attempt: `cmd.exe /c "set ARIAD_NO_BROWSER=1&& set ARIAD_EXIT_ON_READY=1&& call OPEN-ARIAD.cmd"` followed by launcher-log / frontend-log review; passing proof attempt: `cmd.exe /c "set ARIAD_NO_BROWSER=1&& call OPEN-ARIAD.cmd"` launched in a hidden process, then verified `backend_ready=True`, `service_ready=True`, fetched `http://127.0.0.1:3001/api/health`, and fetched `http://127.0.0.1:5173/service.html` while the launcher session was active
...
- Result: The direct-open requirement is now satisfied. `ARIAD.html` no longer asks the user to run `CMD`, open another HTML file, or click `Open Live Service`; opening the root HTML file itself renders the ARIAD workspace and supports real local canvas interaction.
...
- Approval needed: `none`
- Next step: If requested, the next smallest sensible slice is a dedicated viewport/usability pass that decides whether node creation, keyword-cloud opening, and search-driven focus should all share one canonical ?ï¿½bring into view??behavior.
...
- Tests run: `yarn install --mode=skip-build`; `yarn workspace @ariad/frontend typecheck`; `yarn workspace @ariad/backend typecheck`; `yarn workspace @ariad/shared typecheck`; `yarn workspace @ariad/recommendation typecheck`; `yarn typecheck`; attempted `yarn build`
...
- Active milestone: Post-baseline support task
- Agents engaged: Orchestrator
- Touched zones: root workspace, `frontend/`, `backend/`, `recommendation/`, `PLANS.md`
- What changed: Prepared the requested local execution environment for the current Yarn-based workspace by activating Yarn 4.1.1 through Corepack outside the restricted sandbox, installing workspace dependencies with `corepack yarn install --mode=skip-build`, and materializing local `.env` files from the checked-in examples for `frontend`, `backend`, and `recommendation` so the local/default runtime values are explicit. Then ran the current validation family to separate sandbox-only failures from real repository breakage: `typecheck` passed, but `lint` failed on existing frontend React effect-state warnings plus existing root script `no-undef` violations, and the shared/backend/frontend runtime paths all exposed real workspace-package resolution failures around `@ariad/shared` and `@ariad/recommendation`. Direct inspection after install showed the generated `node_modules/@ariad/shared` and `node_modules/@ariad/recommendation` copies contain package manifests that export `./dist/...`, but the installed copies themselves do not contain `dist/`, which matches the build/test/e2e import-resolution failures now seen across the workspace.
- Tests run: `corepack yarn --version`; `corepack yarn install --mode=skip-build`; `corepack yarn lint`; `corepack yarn typecheck`; `corepack yarn test` (inside sandbox, then rerun outside sandbox); `corepack yarn integration`; `corepack yarn build`; `corepack yarn e2e`
- Result: The local Yarn execution environment is now set up and reproducible on this machine, but the full validation suite is not green. `typecheck` passed. `lint` failed on existing repository issues. `test` initially hit sandbox `spawn EPERM`, then outside the sandbox still failed in backend on `@ariad/recommendation` entry resolution. `integration`, `build`, and `e2e` also failed because workspace package imports resolve to installed copies that do not include the exported `dist` outputs.
- Warnings / blockers: Current evidence points to a real Yarn workspace/dependency-linking problem in the repo baseline rather than a remaining machine-setup issue. The internal package dependencies are still declared via local `file:../...` specifiers, and the installed workspace copies under `node_modules/@ariad/*` expose `package.json` entries that point at `dist` files that are absent in those installed copies. Until that resolution path is repaired, backend/frontend build and end-to-end validation will remain blocked even though Yarn itself is installed and runnable.
- Approval needed: `none`

### 2026-04-24 / loop-059
- Active milestone: Post-baseline support task
- Agents engaged: Orchestrator
- Touched zones: `frontend/`, `e2e/`, root standalone `ARIAD.html`, `PLANS.md`
- What changed: Added a new ARIAD marketing landing page at `/`, a product explanation page at `/explanation`, and moved the existing interactive workspace shell to `/workspace` with navigation links between the three surfaces. Added responsive marketing-page styling, route coverage, a Playwright smoke for the new page flow, updated existing workspace e2e coverage to use `/workspace`, rebuilt the frontend bundle, and regenerated the direct-open standalone `ARIAD.html` from the latest build.
- Tests run: `corepack yarn workspace @ariad/frontend lint` (failed on pre-existing `WorkspaceShell.tsx` `react-hooks/set-state-in-effect` findings); `corepack yarn workspace @ariad/frontend typecheck`; `corepack yarn workspace @ariad/frontend test`; `corepack yarn typecheck`; `corepack yarn test`; `corepack yarn build`; `powershell -ExecutionPolicy Bypass -File scripts/generate-standalone-html.ps1`; `corepack yarn e2e`; direct `file:///C:/WORK/ARIAD/ARIAD.html` Playwright smoke for landing -> explanation.
- Result: Landing/explanation/workspace routing is implemented, source validation passed except for the already-known lint baseline, build/e2e are green, and the regenerated standalone ARIAD.html opens to the new landing/explanation flow.
- Warnings / blockers: Lint remains blocked by existing synchronous state updates inside `frontend/src/routes/WorkspaceShell.tsx`, unrelated to this marketing-page slice. Regenerating standalone `ARIAD.html` required building shared/recommendation package outputs first so Yarn's current `file:` workspace copies include their `dist` exports.
- Approval needed: `none`

### 2026-04-24 / loop-060
- Active milestone: Post-baseline support task
- Agents engaged: Orchestrator, Design Director subagent
- Touched zones: `frontend/`, `e2e/`, root standalone `ARIAD.html`, `PLANS.md`
- What changed: Reviewed the new ARIAD landing and explanation pages with a design-director readability pass, then tightened the implementation around that critique. Reduced oversized hero and section typography, loosened tracking/line-height, capped body copy measure, made card copy more scannable, improved mobile CTA hierarchy, shortened mobile hero-board density, added labeled proof cards, added an explanation-page summary strip, and made CTA labels more concrete. Regenerated standalone `ARIAD.html` from the latest build.
- Tests run: `corepack yarn workspace @ariad/frontend lint` (failed on pre-existing `WorkspaceShell.tsx` `react-hooks/set-state-in-effect` findings); `corepack yarn workspace @ariad/frontend typecheck`; `corepack yarn workspace @ariad/frontend test`; `corepack yarn build`; `powershell -ExecutionPolicy Bypass -File scripts/generate-standalone-html.ps1`; `corepack yarn e2e`; direct mobile `file:///C:/WORK/ARIAD/ARIAD.html` Playwright smoke for landing -> explanation.
- Result: Readability-focused design modifications are complete. Focused typecheck, unit, build, e2e, and direct-file smoke validation passed; lint remains blocked only by the known unrelated workspace-shell baseline findings.
- Warnings / blockers: The marketing pages are now more readable, but full repository lint still cannot be claimed green until the pre-existing `WorkspaceShell.tsx` React effect-state lint violations are addressed in a separate workspace-shell cleanup slice.
- Approval needed: `none`

### 2026-04-24 / loop-061
- Active milestone: Post-baseline support task
- Agents engaged: Orchestrator
- Touched zones: `shared/`, `frontend/`, `backend/`, `e2e/`, root standalone `ARIAD.html`, `PLANS.md`
- What changed: Applied the user-directed object-library scope change from project-wide to episode-separated. Added `episodeId` to shared `StoryObject`, create/migration logic that assigns objects to the active episode, active-episode filtering for the object library and inline object mention suggestions, per-episode object pin storage, episode-delete cleanup for episode-owned objects, backend/standalone sync cleanup for episode deletion, and flush-queue dependency ordering so object upserts wait for their episode. Updated UI copy and the object-library header to show the active episode. Added unit/e2e coverage proving an object created in Episode 12 disappears in Episode 11 and returns when Episode 12 is selected. Regenerated standalone `ARIAD.html` from the latest build.
- Tests run: `corepack yarn typecheck`; `corepack yarn test`; `corepack yarn integration`; `corepack yarn build`; `powershell -ExecutionPolicy Bypass -File scripts/generate-standalone-html.ps1`; `corepack yarn e2e`; direct `file:///C:/WORK/ARIAD/ARIAD.html#/workspace` Playwright smoke for object creation and episode switching; `git diff --check`; `corepack yarn lint` (failed on pre-existing workspace-shell effect-state findings and root script `no-undef` findings).
- Result: Episode-scoped object libraries are implemented and validated across frontend, backend persistence, e2e, and standalone direct-open output. Build, tests, integration, e2e, direct smoke, typecheck, and diff whitespace checks passed.
- Warnings / blockers: This intentionally changes the earlier project-wide object model per direct user instruction and touches shared contracts, so rollback would require reverting the `StoryObject.episodeId` contract plus the frontend/backend scope filtering and cleanup changes. Full lint remains blocked by known pre-existing lint issues outside the core behavior validated here.
- Approval needed: `none`
...
- Agents engaged: Orchestrator
- Touched zones: `frontend/`, `e2e/`, root standalone `ARIAD.html`, `PLANS.md`
- What changed: Completed the requested inline-object finalization and end-node shrink repair. Inline object mentions can now be finalized three ways: by typing the closing `@` as before, by pressing Space after an open `@object` mention, or by pressing Enter after an open `@object` mention. Space finalization preserves a trailing space, while Enter finalization inserts a newline after the completed `@object@` token. Updated selection handling so suggestion picks and typed mention names share the same finalization path. Reworked selected-node height syncing so automatically grown nodes can shrink again when their text becomes shorter, while manually resized nodes keep their explicit size floor; timeline-end positioning is refreshed when the end major node auto-resizes. Added unit coverage for Space/Enter object finalization and e2e coverage proving the end major node remains anchored and shrinks after long text is reduced. Regenerated standalone `ARIAD.html` from the latest build.
- Tests run: `corepack yarn workspace @ariad/frontend typecheck`; `corepack yarn e2e --grep "keeps the timeline end node"`; `corepack yarn typecheck`; `corepack yarn test`; `corepack yarn integration`; `corepack yarn build`; `powershell -ExecutionPolicy Bypass -File scripts/generate-standalone-html.ps1`; `corepack yarn e2e`; `corepack yarn lint`.
- Result: The requested three-way object finalization and end-node shrink behavior are implemented. Typecheck, unit tests, integration tests, production build, standalone generation, and the full Playwright e2e suite passed.
- Warnings / blockers: `corepack yarn lint` still fails on the repository's existing `WorkspaceShell.tsx` React effect-state lint pattern plus root script global `no-undef` findings. Those lint findings predate this behavior slice as a broader lint-baseline cleanup issue, so lint cannot be claimed green yet.
- Approval needed: `none`
...
- What changed: Fixed keyword-cloud ordering so selecting or deselecting keyword chips no longer rearranges the cloud immediately. The visible cloud now keeps the suggestion order from the moment it was opened, updates only the selected/pressed state during keyword toggles, and applies the selected-keyword-first pinning plus refresh rotation only when the user explicitly refreshes the cloud. Regenerated standalone `ARIAD.html` from the latest build.
- Tests run: `corepack yarn workspace @ariad/frontend test --run WorkspaceShell.test.tsx`; `corepack yarn workspace @ariad/frontend typecheck`; `corepack yarn build`; `powershell -ExecutionPolicy Bypass -File scripts/generate-standalone-html.ps1`; `corepack yarn e2e`.
- Result: The keyword cloud now stays visually stable while selecting keywords and only reorders after Refresh Cloud. Targeted frontend coverage, frontend typecheck, build, standalone generation, and full e2e passed.
- Warnings / blockers: Full repository lint was not rerun for this small UI-order slice; the known lint baseline from prior loops remains unresolved until the broader `WorkspaceShell.tsx` effect-state and root script global findings are cleaned up.
- Approval needed: `none`
...
- Agents engaged: Orchestrator
- Touched zones: `frontend/`, root standalone `ARIAD.html`, `PLANS.md`
- What changed: Expanded selected-node resizing from the old right / bottom / bottom-right handles to eight resize handles around the full node outline: top, right, bottom, left, and all four corners. Left/top-side resizing now uses live placement overrides while dragging so the opposite edge stays visually anchored, then persists the adjusted placement when the resize gesture ends. Also stopped keyword-cloud clicks from moving the viewport back to the node by removing the editor focus/selection side effect from keyword toggles; clicking a cloud keyword now only updates selection state and node content. Regenerated standalone `ARIAD.html` from the latest build.
- Tests run: `corepack yarn workspace @ariad/frontend test --run WorkspaceShell.test.tsx`; `corepack yarn workspace @ariad/frontend typecheck`; `corepack yarn build`; `powershell -ExecutionPolicy Bypass -File scripts/generate-standalone-html.ps1`; `corepack yarn e2e`; `git diff --check`.
- Result: Nodes can now be resized from every side/corner, keyword-cloud selection no longer yanks focus/view back to the node, targeted frontend coverage passed, frontend typecheck passed, build/e2e passed, standalone output was refreshed, and diff whitespace checks passed.
- Warnings / blockers: Full repository lint was not rerun for this focused UI interaction slice; the known lint baseline from prior loops remains unresolved until the broader `WorkspaceShell.tsx` effect-state and root script global findings are cleaned up.
- Approval needed: `none`

### 2026-04-24 / loop-065
- Active milestone: Post-baseline support task
- Agents engaged: Orchestrator
- Touched zones: root workspace, `shared/`, `recommendation/`, `backend/`, `frontend/`, `scripts/`, `e2e/`, `PLANS.md`
- What changed: Added drag-to-folder behavior in the left episode list so an episode can be dragged directly onto a folder card to move it into that folder, while preserving the existing folder sort mode for reordering inside a folder. Renamed the former project naming throughout the repository from the old name to ARIAD, including visible copy, docs, workspace package names/imports now under `@ariad/*`, environment variable names, launcher helper filenames (`OPEN-ARIAD.*`), runtime standalone flags, and Yarn lock/install state. Adjusted the blank object-library empty-state so the ¡°No reference objects match this episode search yet.¡± message spans the full object grid and stays on one line. Regenerated standalone `ARIAD.html` from the latest build.
- Tests run: `corepack yarn install --mode=skip-build`; `corepack yarn typecheck`; `corepack yarn test`; `corepack yarn integration`; `corepack yarn build`; `powershell -ExecutionPolicy Bypass -File scripts/generate-standalone-html.ps1`; `corepack yarn e2e` (first run failed against a stale reused dev server on port 4173; after stopping the stale process, rerun passed); `git diff --check`; `corepack yarn lint`.
