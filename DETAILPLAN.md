...
## detail-028 / 노드 이동 안정화 종합 점검 계획

이 섹션은 노드 이동 중 발생하는 순간이동, 주변 노드 동반 이동, 드래그 후 예측 불가능한 재배치 문제를 한 번에 정리하기 위한 인수인계 계획입니다.

현상 요약:

- 노드를 움직일 때 특정 노드가 갑자기 아래로 튀는 것처럼 보임
- 사용자가 잡은 노드가 아닌 근처 노드가 같이 밀려 내려가거나 따라오는 것처럼 보임
- 이전 reorder 이후 다음 drag에서 다른 노드 위치가 다시 보정되는 느낌이 남음

현재 판단:

- 부모를 이동할 때 자식이 함께 움직이는 것은 `SPEC.md`의 의도된 동작입니다
- 하지만 형제나 단순히 가까운 노드가 함께 이동하는 것은 버그로 봐야 합니다
- 직접 원인은 backend보다 frontend canvas render/drag pipeline에 가깝습니다
- 특히 `resolveNodeOverlapPlacement(...)`, `applyLaneVerticalReflow(...)`, `getInsertIndexForCanvasY(...)`, `moveNodeFreely(...)`가 한 drag 안에서 placement 보정과 구조 reorder를 동시에 처리합니다

### 28.1 Frontend

목표:

- 사용자가 잡은 노드와 의도된 descendant만 움직이게 합니다
- 단순 주변 노드는 실제 겹침 해결이 필요한 경우 외에는 위치가 바뀌지 않게 합니다
- drop 이후 화면 위치, 저장된 `canvasY`, 구조 순서가 서로 크게 어긋나지 않게 합니다

문제 A: 렌더 시점 lane reflow가 너무 넓게 작동

- 현재 render placement 계산 후 같은 레인 전체에 `applyLaneVerticalReflow(...)`가 다시 적용됩니다
- major lane은 start major만 locked 처리되고, minor/detail lane은 locked node 없이 전체 레인이 보정됩니다
- 이 로직은 구조 순서를 보존하면서 아래쪽 노드를 밀기 때문에, 사용자는 “근처 노드가 같이 따라왔다”고 느낄 수 있습니다

방향 A:

- drag 중에는 active node 외 기존 노드를 기본 locked로 보고, 실제 overlap이 확인된 최소 범위만 reflow합니다
- minor/detail에도 active drag 상황의 `lockedNodeIds` 또는 affected range 개념을 적용합니다
- reflow helper는 유지하되, 호출자가 “전체 레인 정렬”과 “drag 중 겹침 완화”를 구분해서 호출하게 합니다
- 기존 `hasLaneNodeOverlap(...)`를 활용해 겹침이 없는 레인에서는 reflow를 생략합니다

문제 B: drop commit에서 placement 보정과 구조 reorder가 한 번에 일어남

- `moveNodeFreely(...)`는 먼저 placement를 저장한 뒤 `controller.moveNode(...)`로 구조 순서를 바꿉니다
- 그 다음 render pass에서 구조 순서 기반 reflow가 다시 적용되면, 방금 둔 위치가 아래로 튀는 것처럼 보일 수 있습니다
- major node는 timeline snap까지 섞여 있어 jump 체감이 더 큽니다

방향 B:

- drag preview 위치, commit placement, post-render placement를 같은 기준으로 맞춥니다
- drop 직후 active node의 최종 placement를 우선 보존하고, 구조 reorder는 connection/order만 바꾸는 경로로 분리합니다
- 필요한 경우 `moveNodeFreely(...)`에서 reflow된 최종 y를 다시 저장하지 말고, 사용자가 둔 y를 canonical placement로 저장합니다
- major timeline snap은 start/end anchor에 필요한 경우로 제한하고, 일반 major reorder와 end-handle drag를 분리해 검증합니다

문제 C: parent-child 의도와 sibling movement가 구분되지 않음

- `controller.moveNode(...)`는 subtree order를 함께 이동하지만, 실제 descendant placement delta는 별도 명시가 약합니다
- 반대로 lane reflow는 descendant 여부와 상관없이 같은 레인의 뒤쪽 노드를 밀 수 있습니다

방향 C:

- parent drag 시 descendant는 명시적으로 같은 delta만큼 이동시키는 정책을 route/controller 중 한 곳에서 고정합니다
- sibling은 overlap 해결이 필요한 경우가 아니면 위치를 보존합니다
- 테스트에서는 “parent의 child가 함께 이동함”과 “근처 sibling은 움직이지 않음”을 별도 케이스로 분리합니다

파일 후보:

- `frontend/src/routes/WorkspaceShell.tsx`
- `frontend/src/routes/workspace-shell/workspaceShell.canvas.ts`
- `frontend/src/routes/workspace-shell/workspaceShell.layout.test.ts`
- `frontend/src/routes/WorkspaceShell.test.tsx`
- 필요 시 `frontend/src/persistence/controller.ts`
- 필요 시 `frontend/src/persistence/nodeTree.ts`
...
- reorder 이후 major node 하나를 드래그해도 다른 major sibling이 불필요하게 내려가지 않음
- minor/detail node를 같은 레인 sibling 근처로 드래그해도 겹치지 않는 sibling 위치가 유지됨
- parent major를 이동하면 연결된 minor/detail descendant만 같은 delta로 이동함
- minor를 이동하면 연결된 detail descendant만 같은 delta로 이동함
- explicit rewire 후 drag해도 parent가 조용히 바뀌지 않음
- drop 직후 node top이 preview top과 큰 차이 없이 유지됨
- 새로고침 후 저장된 placement가 렌더 reflow 때문에 다시 크게 바뀌지 않음

### 28.2 Backend

목표:

- 이번 이슈의 1차 원인은 아니므로 schema/API 변경 없이 보존성만 확인합니다
- FE가 보낸 node placement, order, parent 정보를 backend canonicalization이 임의로 덮어쓰지 않는지 검증합니다

방향:

- 기존 backend normalize/canonicalization은 `canvasX`, `canvasY`, `parentId`를 보존해야 합니다
- FE에서 descendant placement delta를 여러 node update로 보낼 경우 기존 persistence route가 모두 보존하는지 smoke로 확인합니다
- backend는 movement semantics를 새로 판단하지 않습니다
- parent inference 정책을 backend로 옮기는 변경은 이번 범위에서 제외합니다

파일 후보:

- 직접 수정 없음
- 필요 시 `backend/src/persistence/node-order.test.ts`
- 필요 시 `backend/src/persistence/routes.integration.test.ts`

검증 포인트:

- sparse reorder/canonicalization 후 `canvasX`, `canvasY`, `parentId`가 보존됨
- 여러 node placement upsert가 들어와도 reload 후 위치가 유지됨
- backend가 sibling movement를 새로 만들어내지 않음

### 28.3 병합 순서

1. Frontend route test로 현재 증상을 먼저 재현합니다
2. `workspaceShell.canvas.ts`에서 reflow 범위와 locked 정책을 좁힙니다
3. `WorkspaceShell.tsx`의 drag preview / commit / render placement 기준을 맞춥니다
4. parent-descendant 이동과 sibling 보존 테스트를 추가합니다
5. 필요할 때만 controller placement update 보조를 적용합니다
6. backend는 최종 smoke로 placement 보존만 확인합니다

### 28.4 리스크

- 전체 reflow를 너무 줄이면 실제 겹침이 생길 수 있습니다
- parent inference 정책을 바꾸면 제품 의미와 연결 규칙에 닿을 수 있으므로, 이번 작업은 우선 시각적 이동 안정화에 한정합니다
- `WorkspaceShell.tsx`는 고충돌 파일이므로 이 작업 중에는 single-writer로 다루는 편이 안전합니다
- 현재 frontend 기본 test script에는 빈 워크스페이스 정책 이후 낡은 기대값이 남아 있어, 관련 route/layout test를 명시 실행해야 정확한 신호를 볼 수 있습니다

## detail-029 / Lane Array + Spacer 기반 노드 이동 안정화 재계획

이 섹션은 `detail-028`의 후속 재계획입니다.
사용자 제안인 “각 레인을 배열로 보고, 노드와 빈공간 컴포넌트로 구성하는 방식”을 채택하되,
1차 구현은 frontend 내부 레이아웃 모델 변경으로 제한합니다.

핵심 결정:

- 레인을 `node`와 `spacer` block의 배열로 표현합니다
- `spacer`는 스토리 노드가 아니라 시각적 간격/드롭 위치를 표현하는 layout-only block입니다
- backend/shared contract는 1차에서 변경하지 않습니다
- 저장은 기존 `canvasX`, `canvasY`, `orderIndex`, `parentId`를 계속 사용합니다
- frontend가 lane array를 만들고, drag/drop 후 다시 기존 node placement로 역산합니다

이 접근의 목적:

- 현재처럼 자유좌표, overlap 보정, lane-wide reflow, 구조 reorder가 동시에 작동하는 문제를 줄입니다
- “무엇이 이동했는지”를 배열 조작으로 명확히 만듭니다
- 빈공간을 명시적으로 다뤄 노드가 갑자기 아래로 튀는 체감을 줄입니다
- 부모-자식 이동과 형제 노드 보존을 분리해 테스트하기 쉽게 만듭니다

### 29.1 Frontend

#### 목표

- 각 lane을 `LaneLayoutBlock[]`로 계산합니다
- drag 중에는 실제 DOM 위치를 좌표 탐색보다 block reorder/resize로 설명합니다
- 기존 전체 lane reflow를 대체하거나 최소화합니다
- 사용자는 canvas처럼 느끼되, 내부 동작은 배열 기반으로 안정화합니다
...
- `LaneLayoutBlock`
  ...
  - `id`
  - `height`
  - `nodeId`는 `kind: "node"`일 때만 존재
  - `role: "gap" | "drop-preview"`는 `kind: "spacer"`일 때만 존재
...
- spacer는 selectable/editable node가 아닙니다
- spacer에는 사용자-facing text를 넣지 않습니다
- spacer는 필요하면 drop target 또는 preview zone으로만 동작합니다

#### 단계 A. lane layout 계산 helper 추가

역할:

- 기존 `visibleNodes`, `nodeSizes`, `canvasY`, lane bounds에서 lane별 block 배열을 만듭니다
- node 사이의 실제 간격을 spacer height로 변환합니다
- 겹침이 있으면 최소 gap을 spacer로 보정하되, 전체 노드 y를 임의로 밀지 않습니다

파일 후보:

- `frontend/src/routes/workspace-shell/workspaceShell.laneLayout.ts`
- `frontend/src/routes/workspace-shell/workspaceShell.laneLayout.test.ts`
- 필요 시 `frontend/src/routes/workspace-shell/workspaceShell.canvas.ts`

검증:

- `canvasY`가 있는 node들이 `node/spacer/node` 배열로 안정적으로 변환됨
- 저장된 큰 간격이 spacer height로 유지됨
- 겹친 node는 최소 spacer 보정으로만 분리됨
- 배열을 다시 placement map으로 변환했을 때 node top이 예측 가능함

#### 단계 B. render placement를 lane array 기반으로 전환

역할:

- `WorkspaceShell.tsx`의 `nodePlacements` 계산을 lane layout 결과로 바꿉니다
- 기존 `applyLaneVerticalReflow(...)` 전체 호출은 제거하거나 fallback으로만 남깁니다
- major timeline anchor 규칙은 lane array 위에 별도 적용합니다
...
- 새로고침 후 node top이 저장된 간격과 일관됨
- minor/detail이 아래로 길어져도 major timeline end는 major 기준을 유지함
- lane divider resize 후에도 spacer 기반 배치가 깨지지 않음

#### 단계 C. 빈공간 컴포넌트 도입

역할:

- spacer block을 DOM에서 안정적인 높이로 렌더링합니다
- 평상시에는 보이지 않거나 매우 낮은 contrast의 drop affordance만 가집니다
- drag 중에는 drop-preview spacer로 확장될 수 있습니다

파일 후보:

- `frontend/src/routes/workspace-shell/CanvasLaneSpacer.tsx`
- `frontend/src/routes/workspace-shell/CanvasNodeCard.tsx`
- `frontend/src/styles.css`

검증:

- spacer가 node처럼 선택/편집/삭제되지 않음
- spacer 때문에 카드 hover/selection/connection handle이 흔들리지 않음
- drag 중 preview spacer가 레이아웃을 설명하지만 사용자-facing copy를 추가하지 않음

#### 단계 D. drag/drop을 배열 조작으로 변경

역할:

- drag 중 pointer y를 lane block index와 spacer height 변경으로 변환합니다
- drop 시 `node block` 또는 `subtree block group`을 배열 내 새 위치로 이동합니다
- 최종 lane array에서 `canvasY`를 다시 계산해 `updateNodePlacement(...)`로 저장합니다
- 구조 순서 변경이 필요할 때만 `controller.moveNode(...)`를 호출합니다

중요 정책:

- 같은 레인 sibling은 실제로 block 위치가 바뀐 경우에만 이동합니다
- 겹침 해소를 위해 주변 노드를 무조건 밀지 않습니다
- parent 이동은 descendant 이동 transaction으로 처리합니다
- descendant는 각자의 lane에서 같은 delta만큼 spacer를 조정해 이동합니다
...
- major parent를 이동하면 연결된 minor/detail descendant만 함께 이동함
- minor parent를 이동하면 연결된 detail descendant만 함께 이동함
- unrelated sibling은 drag/drop 후 top이 유지됨
- drop preview 위치와 commit 후 위치 차이가 작음
- explicit rewire 후 이동해도 parent가 조용히 바뀌지 않음

#### 단계 E. 테스트 게이트

필수:

- `corepack yarn --cwd frontend run -T vitest run src/routes/workspace-shell/workspaceShell.laneLayout.test.ts`
- `corepack yarn --cwd frontend run -T vitest run src/routes/workspace-shell/workspaceShell.layout.test.ts`
- `corepack yarn --cwd frontend run -T vitest run src/routes/WorkspaceShell.test.tsx`
- `corepack yarn --cwd frontend run -T tsc -p tsconfig.app.json --noEmit`
- 변경 파일 대상 eslint

추가 권장:

- 실제 브라우저 smoke로 major/minor/detail drag를 한 번 확인합니다
- 필요 시 e2e smoke에 “sibling stays put” 케이스를 추가합니다

### 29.2 Backend

#### 1차 목표

- backend는 새 lane array 모델을 저장하지 않습니다
- 기존 node 저장 필드가 frontend의 lane-array 결과를 그대로 보존하는지만 확인합니다
- schema/shared contract 변경은 하지 않습니다
...
- 여러 node의 `canvasY`가 한 번에 바뀌어도 sync/reload 후 유지됨
- sparse reorder canonicalization이 `canvasY`를 바꾸지 않음
- parent/child 이동 후 여러 node placement가 저장되어도 reload 후 유지됨
...
lane array 자체를 저장하는 방안은 1차 안정화 후 별도 결정합니다.

만약 2차에서 저장 구조를 바꾸면:

- shared contract 변경
- backend persistence schema/API 변경
- migration 또는 backward compatibility
- Version Control gate
- human approval

이 필요합니다.

현재 추천은 2차를 보류하고, 1차에서는 기존 저장 구조를 유지하는 것입니다.

### 29.3 병합 순서

1. Frontend lane layout helper와 단위 테스트를 먼저 추가합니다
2. 기존 render placement를 lane array 기반으로 교체합니다
3. spacer 컴포넌트를 도입하되 사용자-facing node처럼 보이지 않게 합니다
4. drag/drop commit을 block 이동 기반으로 바꿉니다
5. parent-descendant 이동 transaction을 고정합니다
6. route regression으로 sibling 보존과 preview/drop 안정성을 확인합니다
7. backend는 보존성 smoke만 실행합니다

### 29.4 리스크

- 내부 모델 변경이 커서 `WorkspaceShell.tsx` 충돌 위험이 큽니다
- spacer를 잘못 노출하면 사용자가 empty node로 오해할 수 있습니다
- 너무 배열식으로 고정하면 `SPEC.md`의 rough vertical story-position 감각이 약해질 수 있습니다
- 이를 막기 위해 spacer height를 통해 자유 간격을 유지해야 합니다
- 1차에서 backend contract를 바꾸면 범위가 커지므로 반드시 피해야 합니다

### 29.5 결정

채택합니다.

단, 1차 구현 범위는 frontend 내부 lane array + spacer 모델로 제한합니다.
backend/shared 저장 모델 변경은 하지 않습니다.
이 방식이 안정화되면 그때 lane layout persistence를 별도 안건으로 검토합니다.

## detail-030 / 노드 크기 DB 저장 계획

이 섹션은 `detail-029`의 후속 DB 계획입니다.
lane array와 spacer 자체는 저장하지 않고, spacer 계산에 필요한 노드 크기만 canonical persistence에 포함합니다.

핵심 결정:

- DB에 lane array를 저장하지 않습니다
- DB에 spacer block을 저장하지 않습니다
- 각 노드의 실제 카드 크기만 저장합니다
- 권장 필드명은 `canvasWidth`, `canvasHeight`입니다
- 기존 `canvasX`, `canvasY`, `orderIndex`, `parentId`와 함께 node layout을 복원합니다

이유:

- spacer 높이는 `next.canvasY - previous.canvasY - previous.canvasHeight`로 계산됩니다
- `canvasHeight`가 localStorage에만 있으면 새로고침, 다른 브라우저, 다른 계정에서 spacer 계산이 달라질 수 있습니다
- node size를 DB에 저장하면 lane array는 frontend 내부 모델로 유지하면서도 레이아웃 복원이 안정됩니다

### 30.1 Version Control Gate

이 작업은 다음 경계를 건드립니다.

- shared type 변경
- backend persistence schema 변경
- MySQL table/backfill 변경
- frontend persistence/controller 연동 변경
- rollback-sensitive layout migration

따라서 구현 전 Version Control gate가 필요합니다.
단, 제품 의미를 바꾸는 작업은 아니므로 현재 판단상 hard-stop approval 대상은 아닙니다.

필요한 rollback 기준:

- 새 필드는 optional로 추가합니다
- 기존 데이터에는 기본 카드 크기(`nodeCardWidth`, `nodeCardHeight`)를 fallback으로 사용합니다
- 배포 중 일부 클라이언트가 새 필드를 모르는 상태여도 기존 `canvasX`, `canvasY` 기반 렌더가 동작해야 합니다

### 30.2 Shared

목표:

- `StoryNode`에 optional size 필드를 추가합니다
- 저장/전송 contract에서 node size를 명시합니다
...
- 필요 시 `shared/src/types/persistence.ts`
- 필요 시 `shared/src/contracts/persistence.ts`
- 관련 generated `.js`, `.d.ts`, `.map` 산출물

검증:

- shared typecheck
- shared test
- frontend/backend/recommendation import 영향 확인

### 30.3 Backend

목표:

- node size를 canonical persistence에 저장하고 reload 시 그대로 복원합니다
- 기존 데이터는 size가 없어도 정상 복원됩니다

DB 변경:

- `cloud_nodes.canvas_width DOUBLE NULL`
- `cloud_nodes.canvas_height DOUBLE NULL`

처리 규칙:

- insert/upsert 시 `node.canvasWidth ?? null`, `node.canvasHeight ?? null` 저장
- read 시 null이면 필드를 생략하고 frontend fallback에 맡김
- normalize/canonicalize/reorder는 `canvasWidth`, `canvasHeight`를 변경하지 않음
- fixed node lock, parent validation, order canonicalization과 size persistence를 섞지 않음

파일 후보:

- `backend/mysql/init/001_create_cloud_projects.sql`
- `backend/src/persistence/mysql-store.ts`
- `backend/src/persistence/store.ts`
- `backend/src/persistence/node-order.ts`
- `backend/src/persistence/node-order.test.ts`
- `backend/src/persistence/routes.integration.test.ts`

검증:

- `canvasWidth`, `canvasHeight`가 import/sync/reload 후 유지됨
- sparse reorder canonicalization 후에도 node size가 유지됨
- 기존 size 없는 snapshot도 정상 로드됨
- MySQL bootstrap이 기존 DB에 새 nullable columns를 안전하게 추가함

### 30.4 Frontend

목표:

- `nodeSizes` local UI state와 canonical node size를 연결합니다
- 기존 localStorage size는 migration fallback으로만 사용합니다

처리 방향:

- node render size는 `node.canvasWidth/canvasHeight`를 우선 사용합니다
- node에 size가 없으면 기존 `nodeSizes` localStorage 값을 fallback으로 사용합니다
- localStorage에도 없으면 기본 카드 크기를 사용합니다
- resize 종료 시 `nodeSizes` local update와 함께 canonical node size update를 호출합니다
- phase 1에서는 size 변경 저장 API를 기존 node update 경로에 얹고, 별도 lane layout 저장 API는 만들지 않습니다
...
- 노드 resize 후 새로고침해도 크기가 유지됨
- 다른 세션에서 cloud reload 후에도 크기가 유지됨
- lane spacer 계산이 저장된 `canvasHeight` 기준으로 일관됨
- 기존 localStorage size만 있는 사용자는 첫 로드 후 fallback으로 기존 크기를 볼 수 있음
- fixed node resize 차단 정책은 유지됨

### 30.5 병합 순서

1. Version Control gate와 rollback 기준을 먼저 확정합니다
2. shared `StoryNode` optional size 필드를 추가합니다
3. backend schema/store/read/write/test를 추가합니다
4. frontend controller에 node size update 경로를 연결합니다
5. `WorkspaceShell` size source를 `node canonical size -> local fallback -> default` 순서로 정리합니다
6. lane layout helper가 canonical size를 사용하도록 확인합니다
7. frontend/backend focused tests와 build smoke를 실행합니다

### 30.6 리스크

- node size를 저장하면 exact canvas presentation에 가까워지므로, 향후 layout state의 canonical 범위를 더 명확히 해야 합니다
- 기존 localStorage size와 DB size가 충돌할 수 있으므로 우선순위를 고정해야 합니다
- resize 중 매 pointer move마다 DB sync를 보내면 과도하므로, canonical 저장은 resize 종료 시점 또는 debounce가 필요합니다
- schema 변경이므로 rollback과 backward compatibility 테스트를 반드시 포함해야 합니다

### 30.7 결정

채택합니다.

단, 저장 대상은 node size까지만입니다.
lane array/spacer는 계속 frontend 내부 계산 모델로 유지합니다.

## detail-031 / 인라인 작성 중 미등록 오브젝트 생성 복구 계획

이 섹션은 노드 인라인 작성 중 `@` 멘션 메뉴에서 기존 오브젝트에 없는 이름을 바로 오브젝트로 생성하는 기능을 복구하는 계획입니다.

요구 요약:

- 현재 사용자는 `@`를 통해 기존 오브젝트를 검색/삽입할 수 있습니다
- 리스트에 없는 단어를 이어서 입력한 경우, 그 단어를 새 오브젝트로 만들 수 있어야 합니다
- 예전 기능처럼 인라인 작성 흐름을 끊지 않고 새 오브젝트 생성과 노드 연결까지 이어져야 합니다

현재 상태:

- `@Hero...`처럼 기존 오브젝트 prefix와 매칭되면 `.object-mention-menu`에 기존 오브젝트가 표시됩니다
- plain text로 기존 오브젝트 이름을 입력하면 `Use "name" as object` 변환 메뉴가 뜹니다
- `@coat@`처럼 사용자가 닫는 `@`까지 직접 입력하면 `syncObjectMentionsForNode(...)`가 없는 오브젝트를 자동 생성하는 경로는 남아 있습니다
- 하지만 `@newName`처럼 열린 mention query에 기존 매칭 결과가 없으면 overlay 자체가 뜨지 않아 “Create object”를 선택할 수 없습니다

### 31.1 Frontend

목표:

- 열린 `@` mention query에서 검색 결과가 없어도 create option을 표시합니다
- 사용자가 명시적으로 선택했을 때만 새 오브젝트를 생성합니다
- 선택 후 inline text는 기존 object token 형식으로 변환하고, 노드의 `objectIds`에 연결합니다

UX 규칙:

- `@` 뒤 query가 비어 있으면 create option을 보여주지 않습니다
- query가 trim 후 유효하고 같은 이름의 오브젝트가 없으면 `Create "Name" as object` 옵션을 보여줍니다
- 기존 오브젝트가 일부 매칭되는 경우에도 정확히 같은 이름이 없으면 create option을 목록 하단에 보여줄 수 있습니다
- 정확히 같은 이름이 이미 있으면 create option을 숨기고 기존 오브젝트 선택만 제공합니다
- Enter/Tab은 active option을 선택합니다
- Escape 또는 바깥 클릭은 기존처럼 메뉴를 닫습니다
- fixed node에서는 create option도 표시/동작하지 않습니다

텍스트 처리 규칙:

- create option 선택 시 현재 query 범위를 `getObjectToken(name)`으로 치환합니다
- 입력 중인 `@` delimiter는 화면에 남기지 않습니다
- 선택 직후 caret은 삽입된 object token 뒤로 이동합니다
- `syncInlineObjectMentions(...)`를 호출해 object 생성/attach 흐름을 재사용합니다

중복/정규화 규칙:

- 이름 비교는 기존처럼 trim + lowercase 기준입니다
- object name은 사용자가 입력한 display casing을 우선 보존합니다
- 공백만 있는 이름, 줄바꿈 포함 이름, 너무 긴 이름은 create option을 숨기거나 disabled 처리합니다
- 추천 이름에 object token marker가 섞이지 않게 sanitize합니다

파일 후보:

- `frontend/src/routes/WorkspaceShell.tsx`
- `frontend/src/routes/workspace-shell/workspaceShell.inlineEditor.tsx`
- `frontend/src/routes/workspace-shell/workspaceShell.types.ts`
- `frontend/src/routes/workspace-shell/CanvasNodeCard.tsx`
- `frontend/src/routes/WorkspaceShell.test.tsx`
...
1. `objectMentionSuggestions`와 별도로 `objectMentionCreateCandidate`를 계산합니다
2. `objectMentionOverlay` 표시 조건을 `suggestions.length > 0 || createCandidate !== null`로 바꿉니다
3. create option 버튼을 `.object-mention-option` 목록에 추가합니다
4. `applyObjectMentionSelection(...)`를 기존 오브젝트 선택과 새 이름 선택에 모두 재사용합니다
5. 키보드 선택 로직이 suggestions + create option 통합 index를 다루게 합니다
6. route test로 기존 선택, plain word conversion, 새 object creation을 모두 고정합니다

검증:

- `@new coat` 입력 후 `Create "new coat" as object`를 클릭하면 object row가 생성됨
- inline node text에 object mention token이 표시됨
- 생성된 object가 현재 node `objectIds`에 연결되어 usage count가 증가함
- 같은 이름이 이미 있으면 create option 없이 기존 object만 선택됨
- fixed node에서는 create option이 뜨지 않음
- 기존 `@h` 검색으로 `Heroine's Mother`를 선택하는 흐름이 깨지지 않음
- 기존 plain text `Use "Heroine's Mother" as object` 흐름이 깨지지 않음

테스트 후보:

- `corepack yarn --cwd frontend run -T vitest run src/routes/WorkspaceShell.test.tsx -t "creates object from unmatched inline mention query"`
- 관련 helper를 분리하면 `workspaceShell.inlineEditor.test.ts`에 create candidate 계산 테스트 추가
- 변경 파일 대상 eslint
- `corepack yarn --cwd frontend run -T tsc -p tsconfig.app.json --noEmit`
...
- 별도 DB schema 변경 없이 기존 object create/sync 경로를 사용합니다
- frontend가 create option을 통해 만든 object가 기존 persistence contract로 저장되는지만 확인합니다

현재 사용 가능 경로:

- `controller.createObject(...)`
- `controller.attachObjectToNode(...)`
- backend object persistence/sync

변경 필요성:

- 1차 구현에서는 backend 변경이 필요하지 않습니다
- 새 category 선택 UI를 넣지 않는 한 기존 default category `"thing"`을 그대로 사용할 수 있습니다
- object name validation이 backend에 없다면 frontend validation으로 우선 방어합니다

검증:

- 생성된 object가 reload 후 유지됨
- authenticated sync 후 object와 node-object link가 유지됨
- duplicate name 생성이 같은 episode/folder scope에서 중복 object를 만들지 않음

파일 후보:

- 직접 수정 없음
- 필요 시 `backend/src/persistence/routes.integration.test.ts` smoke만 추가

### 31.3 병합 순서

1. Frontend에서 create candidate 계산 helper를 먼저 추가합니다
2. mention overlay 렌더 조건과 create option을 추가합니다
3. mouse/keyboard selection을 suggestions + create option 통합 모델로 정리합니다
4. 기존 `syncInlineObjectMentionsForNode` 생성/attach 경로를 재사용합니다
5. route regression을 추가합니다
6. backend는 기존 persistence smoke만 확인합니다

### 31.4 리스크

- typing 중 자동 생성하면 잘못 입력한 단어가 object로 남을 수 있으므로 반드시 명시 선택 방식이어야 합니다
- 기존 `@name@` 자동 생성 경로와 create option 경로가 중복 생성하지 않도록 normalized name map을 재사용해야 합니다
- object scope가 episode/folder 기준으로 처리되고 있으므로 같은 이름 비교 범위를 기존 로직과 맞춰야 합니다
- 메뉴가 suggestions 0개일 때도 떠야 하므로 접근성 focus/index 계산이 흔들릴 수 있습니다
